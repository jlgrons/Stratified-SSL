run_data_gen_test <- function(old_data, new_data){
  ...
  colnames(old_data$Xt) <- colnames(new_data$covariates_lab)
  colnames(old_data$Xv) <- colnames(new_data$covariates_unlab)
  colnames(old_data$Xr) <- colnames(new_data$covariates_random_samp)
  colnames(old_data$Xvr) <- colnames(new_data$covariates_unlab_random_samp)
  ...
  outcome_test <- all.equal(c(old_data$Y), c(new_data$Y))
