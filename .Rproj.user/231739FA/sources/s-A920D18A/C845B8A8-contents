...
data <- data_gen(n.t, N, p, rho, model.spec = 'CC', strata_num = strata.num)
basis.type <- 'spline'
knots = 3

# # Generate the true regression parameters
# data.v <- data_gen(n.t, 100000-n.t, p, rho, model.spec = 'CC')
# X0.v <- data.v$X0
# Y.v <- data.v$Y
...
# # Generate test data
#
# data.test <- data_gen(n.t = 1000, n.v = 9000, p, rho, model.spec = 'CC', strata_num = strata.num)
#
# X.test <- data.test$X0
# Y.test <- data.test$Y
# S.test <- data.test$S
