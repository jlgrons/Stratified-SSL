...
# Outcome model is wrong, imputation model is correct in Section S4
# (comparing random sample with stratified sampling)
...
data <- data_gen(n.t, N, p, rho, model.spec = 'IC1', strata_num = strata.num)
basis.type <- 'IC1'
knots = 3

# Generate the true regression parameters
data.v <- data_gen(n.t, 100000-n.t, p, rho, model.spec = 'IC1')
X0.v <- data.v$X0
Y.v <- data.v$Y
...
if (strate.ind == 'random'){
  ...
  Yt <- data$Yr
  Xv <- data$Xvr
  samp.prob <- rep(n.t / N, n.t)
  Strata_ID <- data$Sr
  St <- data$Sr
}

if (strate.ind == 'stratify'){
  Xt <- data$Xt
  Yt <- data$Yt
  Xv <- data$Xv
